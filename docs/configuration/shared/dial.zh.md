...
本分支的 Android 客户端会在蜂窝网络上自动使用普通 TCP。网络变化事件更新只读策略，
新连接在首次写入时读取策略，选择预先准备好的 TFO 或普通 TCP 拨号器。
这项策略不增加逐连接的 Android 调用、接口枚举或堆分配；已确认的非蜂窝网络恢复节点原配置。
无法分类的网络、自定义路由标记或网络命名空间暂不使用 TFO。
原本关闭的 `tcp_fast_open` 不会被自动开启，切网沿用现有重置机制，无需重启 VPN。
在系统切网通知送达前开始的连接，可能短暂使用前一个网络的策略。
