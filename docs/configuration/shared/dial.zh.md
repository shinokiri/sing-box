...
本分支的 Android 客户端会在蜂窝网络上自动使用普通 TCP；切回已确认的非蜂窝物理网络后，
新连接恢复节点配置中的 TFO 设置。网络类型在实际创建 socket 时判断，也覆盖切网后才首次写入的延迟连接。
只有使用 TFO 的 socket 才会额外绑定到作出该判断的物理网络，普通 TCP 沿用原有路由。
如果平台无法识别并绑定物理网络，或自定义路由标记、
网络命名空间使该判断不可靠，则暂不使用 TFO。原本关闭的 `tcp_fast_open` 不会被自动开启。
切网时无需为此重启 VPN。
