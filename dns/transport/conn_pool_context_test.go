...
func TestConnPoolDialContextRetirement(t *testing.T) {
	for _, mode := range []ConnPoolMode{ConnPoolSingle, ConnPoolOrdered} {
		for _, retire := range []string{"close", "reset", "invalidate", "release", "dead", "idle"} {
			if mode == ConnPoolOrdered && retire == "idle" { continue }
			t.Run(fmt.Sprintf("mode_%d/%s", mode, retire), func(t *testing.T) {
				...
				if err != nil || !created { t.Fatalf("acquire: created=%v error=%v", created, err) }
				if err := conn.ctx.Err(); err != nil { t.Fatalf("dial context ended before first write: %v", err) }
				switch retire {
				case "close": pool.Close()
				case "reset": pool.Reset()
				case "invalidate": pool.Invalidate(conn, errors.New("broken connection"))
				case "release": pool.Release(conn, false)
				case "idle": pool.Release(conn, true); pool.CloseIdle()
				case "dead":
				...
				select {
				case <-conn.ctx.Done():
				default: t.Fatal("retired connection retained its dial context")
				}
				if !conn.closed.Load() { t.Fatal("retired connection was not closed") }
			})
		}
	}
}
...
func TestConnPoolResetCancelsPendingDial(t *testing.T) {
	for _, mode := range []ConnPoolMode{ConnPoolSingle, ConnPoolOrdered} {
		t.Run(fmt.Sprint(mode), func(t *testing.T) {
			...
			case <-time.After(time.Second): t.Fatal("dial did not start")
			}
			pool.Reset()
			select {
			case <-dialCtx.Done():
			case <-time.After(time.Second): t.Fatal("reset left pending dial running")
			}
			pool.Close()
			select {
			case <-finished:
			case <-time.After(time.Second): t.Fatal("acquire did not exit after reset and close")
			}
		})
	}
}
