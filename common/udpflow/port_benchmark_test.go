...
// Include the TUN-buffer copy, queue, worker, framing-buffer allocation, and
// write completion. No encryption or network latency is included.
func BenchmarkPortForward(b *testing.B) {
	for _, ipv6 := range []bool{false, true} {
		for _, size := range []int{64, 1200} {
			b.Run(fmt.Sprintf("IPv6=%t/payload=%d", ipv6, size), func(b *testing.B) {
