...
func (m testProgressManager) Outbound(tag string) (adapter.Outbound, bool) {
	for _, item := range m.items {
		if item.Tag() == tag { return item, true }
	}
	return nil, false
}
...
func (g testProgressGroup) All() []string {
	var tags []string
	for _, item := range g.items { tags = append(tags, item.Tag()) }
	return tags
}
...
func TestURLTestServiceWaitsForWholeRoundAndJoinsRepeatClick(t *testing.T) {
	...
	if _, err := s.URLTest(ctx, &URLTestRequest{OutboundTag: "all"}); err != nil { t.Fatal(err) }
	for _, node := range []*testProgressNode{a,b} {
		select { case <-node.started: case <-ctx.Done(): t.Fatal(ctx.Err()) }
	}
	snapshot := s.readGroups().Group[0]
	if !snapshot.UrlTestRunning || !snapshot.UrlTestProgressSupported { t.Fatal("running task absent from subscription snapshot") }
	for _, item := range snapshot.Items {
		if item.UrlTestDelay != 0 || item.UrlTestState != urltest.TestRunning { t.Fatal("pending snapshot exposed an old delay") }
		if history.LoadURLTestHistory(item.Tag).Delay != 123 { t.Fatal("presentation change erased routing history") }
	}
	waiting := make(chan error, 1)
	go func() { _, err := s.URLTest(ctx, &URLTestRequest{OutboundTag: "all", Wait: true}); waiting <- err }()
	select { case err := <-waiting: t.Fatalf("request returned before completion: %v", err); case <-time.After(20*time.Millisecond): }
	close(a.release)
	for history.LoadTestStatus("a").State != urltest.TestFailed {
		select { case <-ctx.Done(): t.Fatal(ctx.Err()); case <-time.After(time.Millisecond): }
	}
	partial := s.readGroups().Group[0]
	if !partial.UrlTestRunning || partial.Items[0].UrlTestState != urltest.TestFailed || !history.LoadTestStatus("b").Pending() { t.Fatal("partial completion was presented as a finished round") }
	close(b.release)
	select { case err := <-waiting: if err != nil {t.Fatal(err)}; case <-ctx.Done(): t.Fatal(ctx.Err()) }
	finished := s.readGroups().Group[0]
	if finished.UrlTestRunning || a.calls.Load() != 1 || b.calls.Load() != 1 { t.Fatal("round did not finish once, or duplicate request launched more tests") }
	for _, item := range finished.Items {
		if item.UrlTestState != urltest.TestFailed || item.UrlTestDelay != 0 { t.Fatal("final snapshot retained a successful delay") }
	}
}
