...
func (s *System) Start() error {
	...
	s.dispatcher.EnableAsyncFlow(s.ctx, s.acceptFlowPacket)
	go s.tunLoop()
	return nil
}
...
func (s *System) processIPv4(ipHdr header.IPv4) (writeBack bool, err error) {
	...
	return s.processIPv4Payload(ipHdr)
}

func (s *System) processIPv4Payload(ipHdr header.IPv4) (writeBack bool, err error) {
	writeBack = true
	switch ipHdr.TransportProtocol() {
	case header.TCPProtocolNumber:
...
func (s *System) processIPv6(ipHdr header.IPv6) (writeBack bool, err error) {
	...
	return s.processIPv6Payload(ipHdr)
}

func (s *System) processIPv6Payload(ipHdr header.IPv6) (writeBack bool, err error) {
	writeBack = true
	switch ipHdr.TransportProtocol() {
	case header.TCPProtocolNumber:
...
func (s *System) acceptFlowPacket(packet []byte) {
	var writeBack bool
	var err error
	if header.IPVersion(packet) == header.IPv4Version {
		writeBack, err = s.processIPv4Payload(packet)
	} else {
		writeBack, err = s.processIPv6Payload(packet)
	}
	s.finishAcceptedPacket(packet, writeBack, err)
}

func (s *System) finishAcceptedPacket(packet []byte, writeBack bool, err error) {
	if err != nil {
		s.logger.Trace(err)
		return
	}
	if writeBack {
		writer := s.dispatcher.writeback
		headroom := writer.ReturnHeadroom()
		raw := make([]byte, headroom+len(packet))
		copy(raw[headroom:], packet)
		if err = writer.WriteReturnPackets([][]byte{raw}); err != nil {
			s.logger.Trace(E.Cause(err, "write accepted packet"))
		}
	}
}
