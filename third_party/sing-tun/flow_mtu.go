...
func (s *ForwardDispatcher) resegmentTCP(flow *forwardFlow, packet *forwardPacket, raw []byte, effectiveMTU uint32) {
	...
	if err != nil {
		s.logger.Trace(E.Cause(err, "resegment packet"))
		return
	}
	for i := range n {
		s.stagePort(flow, bufs[i][:sizes[i]])
	}
}

func (s *ForwardDispatcher) reserveSegments(count, size int) ([][]byte, []int) {
	start := s.segmentUsed
	end := start + count
