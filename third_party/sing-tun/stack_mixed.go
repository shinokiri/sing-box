...
func (m *Mixed) Start() error {
	...
	udpForwarder.dispatcher = m.dispatcher
	...
	m.stack = ipStack
	m.endpoint = endpoint
	m.dispatcher.EnableAsyncFlow(m.ctx, m.acceptFlowPacket)
	go m.tunLoop()
	go m.packetLoop()
	return nil
}
...
func (m *Mixed) processIPv4(ipHdr header.IPv4) (writeBack bool, err error) {
	...
	return m.processIPv4Payload(ipHdr)
}

func (m *Mixed) processIPv4Payload(ipHdr header.IPv4) (writeBack bool, err error) {
	writeBack = true
...
func (m *Mixed) processIPv6(ipHdr header.IPv6) (writeBack bool, err error) {
	...
	return m.processIPv6Payload(ipHdr)
}

func (m *Mixed) processIPv6Payload(ipHdr header.IPv6) (writeBack bool, err error) {
	writeBack = true
	switch ipHdr.TransportProtocol() {
	case header.TCPProtocolNumber:
...
func (m *Mixed) acceptFlowPacket(packet []byte) {
	var writeBack bool
	var err error
	if header.IPVersion(packet) == header.IPv4Version {
		writeBack, err = m.processIPv4Payload(packet)
	} else {
		writeBack, err = m.processIPv6Payload(packet)
	}
	m.finishAcceptedPacket(packet, writeBack, err)
}
